import { buildQrPayload, parseMailto, parseSms } from "./src/qr-payloads.js";
...
const QR_QUIET_ZONE_RATIO = 0.045;
const presetSwatchUrls = [];
let previewRenderId = 0;

const getQrMargin = (size) => Math.max(16, Math.ceil(size * QR_QUIET_ZONE_RATIO));
const getPresetQrMargin = (size) => Math.max(8, Math.ceil(size * QR_QUIET_ZONE_RATIO));
...
const getCompiledQRData = () => {
    ...
    return buildQrPayload(type, getQrFormValues()) || " ";
};
...
const getQrErrorCorrectionLevel = () => currentLogo ? "H" : "Q";
...
const getQrConfig = (size = QR_PREVIEW_SIZE, renderType = "svg", margin = getQrMargin(size)) => {
    ...
    const dotsOptions = {
        ...
        // Keep the module geometry locked to the QR grid to avoid visual gaps.
        roundSize: false,
    ...
    return {
        ...
        qrOptions: {
            typeNumber: 0,
            errorCorrectionLevel: getQrErrorCorrectionLevel()
        },
...
const createQrInstance = (size = QR_PREVIEW_SIZE, renderType = "svg", margin) => {
    return new window.QRCodeStyling(getQrConfig(size, renderType, margin));
};
...
const expandModuleRects = (svg) => {
    ...
    const rects = svg.querySelectorAll('[data-qr-module-layer="dots"] rect, clipPath rect');
    const overlap = 0.2;
...
const expandModuleLayer = (svg) => {
    if (dotStyleInput.value === "square") return;

    const moduleLayer = svg.querySelector('[data-qr-module-layer="dots"]');
    ...
    const cy = y + height / 2;
    const scale = 1.028;
    moduleLayer.setAttribute(
        "transform",
        `translate(${cx} ${cy}) scale(${scale}) translate(${-cx} ${-cy})`
    );
};

const getClipPathId = (value) => {
    const match = String(value || "").match(/^url\(["']?#([^"')]+)["']?\)$/);
    return match ? match[1] : "";
};

const getQrLayerName = (clipId) => {
    if (clipId === "clip-path-dot-color") return "dots";
    if (clipId.startsWith("clip-path-corners-square-color")) return "corner-squares";
    if (clipId.startsWith("clip-path-corners-dot-color")) return "corner-dots";
    if (clipId === "clip-path-background-color") return "background";
    return clipId.replace(/^clip-path-/, "");
};

const flattenPaintedClipPaths = (svg) => {
    const paintRects = [...svg.querySelectorAll("rect[clip-path]")];
    paintRects.forEach((paintRect) => {
        const clipId = getClipPathId(paintRect.getAttribute("clip-path"));
        if (!clipId) return;

        const clipPath = svg.querySelector(`clipPath[id="${clipId}"]`);
        if (!clipPath) return;

        const paint = paintRect.getAttribute("fill") || "currentColor";
        const layer = svg.ownerDocument.createElementNS("http://www.w3.org/2000/svg", "g");
        layer.setAttribute("data-qr-module-layer", getQrLayerName(clipId));
        layer.setAttribute("fill", paint);
        layer.setAttribute("clip-rule", "evenodd");
        layer.setAttribute("fill-rule", "evenodd");

        [...clipPath.children].forEach((node) => {
            layer.appendChild(node.cloneNode(true));
        });

        paintRect.replaceWith(layer);
        clipPath.remove();
    });
};

const ensureReferencedPaintsStayInDefs = (svg) => {
    // Keep gradients in <defs>, but remove unused clipping definitions after vectorization.
    svg.querySelectorAll("defs clipPath").forEach((clipPath) => clipPath.remove());
};

const normalizeSvgText = (svgText) => {
    if (typeof DOMParser === "undefined") return svgText;
    const parser = new DOMParser();
    const normalizedRefs = svgText.replace(/url\('#([^']+)'\)/g, "url(#$1)");
    const doc = parser.parseFromString(normalizedRefs, "image/svg+xml");
    ...
    flattenPaintedClipPaths(svg);
    ensureReferencedPaintsStayInDefs(svg);
    expandModuleRects(svg);
    expandModuleLayer(svg);
...
const renderPreview = async () => {
    ...
    previewImg.addEventListener("error", () => URL.revokeObjectURL(previewUrl), { once: true });
    qrElement.appendChild(previewImg);
};
...
// ─── Presets ───────────────────────────────────────────
// Presets favor common commercial use cases: dark modules, light backgrounds, strong contrast.
const presets = {
    // Corporate: dependable business palette for cards, invoices and B2B material.
    corporate: {
        dot: "#172554", dot2: "#1d4ed8", grad: "linear", style: "rounded",
        bg: "#ffffff", trans: false, corner: "#172554", sq: "extra-rounded", dt: "dot"
    },
    // Classic: maximum scanner compatibility for print and operations.
    minimal: {
        dot: "#020617", dot2: "#020617", grad: "none", style: "square",
        bg: "#ffffff", trans: false, corner: "#020617", sq: "square", dt: "square"
    },
    // Hospitality: warm, readable styling for menus, hotels and events.
    midnight: {
        dot: "#7c2d12", dot2: "#b45309", grad: "linear", style: "rounded",
        bg: "#fff7ed", trans: false, corner: "#7c2d12", sq: "extra-rounded", dt: "dot"
    },
    // Eco: green brand palette for wellness, sustainability and food.
    sage: {
        dot: "#14532d", dot2: "#15803d", grad: "linear", style: "extra-rounded",
        bg: "#f0fdf4", trans: false, corner: "#14532d", sq: "extra-rounded", dt: "dot"
    },
    // Tech Blue: common SaaS/product palette with high contrast.
    ocean: {
        dot: "#0f172a", dot2: "#2563eb", grad: "linear", style: "rounded",
        bg: "#eff6ff", trans: false, corner: "#0f172a", sq: "extra-rounded", dt: "dot"
    },
    // Boutique: soft premium retail and personal brand palette.
    rose: {
        dot: "#881337", dot2: "#be123c", grad: "linear", style: "classy",
        bg: "#fff1f2", trans: false, corner: "#be123c", sq: "extra-rounded", dt: "dot"
    },
    // Retail Amber: attention-grabbing but still dark-on-light for promos.
    sunset: {
        dot: "#78350f", dot2: "#b45309", grad: "linear", style: "dots",
        bg: "#fff7ed", trans: false, corner: "#c2410c", sq: "extra-rounded", dt: "dot"
    },
    // Professional: neutral, versatile and close to default UI palettes.
    slate: {
        dot: "#334155", dot2: "#475569", grad: "linear", style: "extra-rounded",
        bg: "#f8fafc", trans: false, corner: "#334155", sq: "extra-rounded", dt: "dot"
    },
    // Luxury: premium black-and-gold look without inverted scan risk.
    noir: {
        dot: "#111827", dot2: "#92400e", grad: "linear", style: "classy",
        bg: "#fffbeb", trans: false, corner: "#111827", sq: "extra-rounded", dt: "dot"
    }
};
...
const renderPresetSwatches = async () => {
    ...
    for (const btn of document.querySelectorAll('.preset-btn')) {
        ...
        try {
            const presetQr = new window.QRCodeStyling(buildPresetQrConfig(preset, PRESET_SWATCH_SIZE, "canvas"));
...
const getCurrentFormValues = () => getQrFormValues();
...
const loadHistoryItem = (item) => {
    ...
    if (item.type === "url") {
        ...
        const sms = values.sms || parseSms(item.data);
